...
WordGenerator.prototype._preparePathCounts = function (length) {
	...
	for (var i = 0; i < length; i++) {
		pathCounts.push(new Array(27 * 27).fill(0n));
	}

	for (var n = 0; n < 27 * 27; n++) {
		if (trigrams[n] & 1) {
			pathCounts[length - 1][n] = 1n;
		}
	}
	...
	for (var level = length - 2; level >= 0; level--) {
		...
		for (var n = startN; n < endN; n++) {
			...
			for (var x = 1; x < 27; x++) {
				if (trigrams[n] & (1 << x)) {
					count += pathCounts[level + 1][base + x];
				}
			}
...
WordGenerator.prototype.nBits = function (length) {
	this._preparePathCounts(length);
	return Math.log2(Number(this.totalPaths));
}

WordGenerator.prototype._randBelow = function (limit) {
	var bits = limit.toString(2).length;
...
WordGenerator.prototype.generate = function (length) {
	var pathCounts = this._preparePathCounts(length);
	var path = this._randBelow(this.totalPaths);

	var grams = [];
	for (var level = 0; level < length; level++) {
		if (level === 0) {
			for (var n = 0; n < 27; n++) {
				if (pathCounts[0][n] <= path) {
					path -= pathCounts[0][n];
				} else {
					grams.push(n);
					break;
			...
			var base = grams[level - 1] % 27 * 27;
			for (var x = 0; x < 27; x++) {
				if (trigrams[grams[level - 1]] & (1 << x)) {
					var n = base + x;
					if (pathCounts[level][n] <= path) {
						path -= pathCounts[level][n];
					} else {
						grams.push(n);
						break;
					}
	...
	return grams.map(g => String.fromCharCode((g % 27) + 96)).join('');
}

WordGenerator.prototype.makeTable = function (length, count) {
	var words = Array.from({ length: count }, () => this.generate(length));

	var bits = this.nBits(length);
	var output = 'Generating ' + count + ' ' + length + '-letter words with ' +
		bits.toFixed(1) + ' bits of entropy\n\n';

	var cols = Math.max(1, Math.floor((80 - length) / (2 + length)) + 1);
	for (var i = 0; i < words.length; i += cols) {
		output += words.slice(i, i + cols).join('  ') + '\n';
	}
